...
Describe "Suggestion Tests" {
    BeforeAll {
        ...
        $singleton = [PSAdapterProvider.PSAdapterFeedbackPredictor]::Singleton
        ...
        $Ast3 = { arp | arp-adapter }.Ast.Find({$args[0] -is [System.Management.Automation.language.CommandAst]}, $false)
        ...
        '"output"' > "${TESTDRIVE}/arp-adapter.ps1"
    ...
    It "Should provide an arp-adapter suggestion for '$ast3'" {
        $suggestions = $generator.GetSuggestions($Ast3)
        $suggestions | Should -Contain "arp | arp-adapter"
    }
